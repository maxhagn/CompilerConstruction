...
void asmMoveValueHeap(long numValue, long offset) {
    offset = (offset + 1) * 8;
    fprintf(stdout, "\tmovq\t$%ld, %ld(%%r15)\n", numValue, offset);
}

void asmMoveRegisterHeap(char *sourceRegister, long offset) {
    offset = (offset + 1) * 8;
    fprintf(stdout, "\tmovq\t%%%s, %ld(%%r15)\n", sourceRegister, offset);
}
